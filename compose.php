...
if(@$_REQUEST['send'])
{
	...
	{
	$msg = str_replace ("'","\'",$msg);
	$d=$conn->query("SELECT * FROM userinfo where emailid='$to'");
	$row=$d->num_rows;
	if($row==1)
		{
		$conn->query("INSERT INTO usermail(to_id,from_id,subject,msg,send_date) values('$to','$id','$sub','$msg',CURRENT_TIMESTAMP)");
		$err= "message sent...".$conn->error;
		}
	else
		{
		$sub=$sub."--"."msg failed";
		$conn->query("INSERT INTO usermail(to_id,from_id,subject,msg,send_date) values('$id','$id','$sub','$msg',CURRENT_TIMESTAMP)");
		$err= "message failed...";
		}	
	}
}	
...
if(@$_REQUEST['save'])
{
	...
	{
	$query="INSERT INTO draft(emailid, subject, msg, send_date) values('$id','$sub','$msg',CURRENT_TIMESTAMP)";
	$conn->query($query);
	$err= "message saved...";
	}
}	
...
<html>
  <head>
    <meta charset="UTF-8">
    <title>Secured Mail Server</title>
...
<table width="506" border="1">
  ...
  <tr>
	...
	<input class="button" type="reset" value="Cancel"/>	</th>
  </tr>
</table>
