...
?> 
	...
	$r=$conn->query("select * from userinfo where emailid='{$_SESSION['sid']}'");
    ...
    <div class="wrapper">
      ...
      <div class="content-wrapper">
		...
		@$coninb=$_GET['coninb'];
			...
			if($coninb)
			{
			$sql="SELECT * FROM usermail where to_id='$id' and mailid=$coninb";
$dd=$conn->query($sql);
			$row=$dd->fetch_object();
	...
	if(isset($_GET['delete']))
	{
		...
		$d="DELETE from usermail where mailid=$v";
		$conn->query($d);
		}
		echo "msg deleted";
	}
	...
	<?php
		...
		@$consent=$_GET['consent'];
			...
			if($consent)
			{
			$sql="SELECT * FROM usermail where from_id='$id' and mailid=$consent";
			$dd=$conn->query($sql);
			$row=$dd->fetch_object();
			echo "Subject :".$row->subject."<br/>";
			echo "Message :".$row->msg;
